...
// SOURCE ENTITIES
    } else if (commands.at(0) == "degree") {
      return_string = std::to_string(g.get_deg((unsigned int) stoi(commands.at(1))));
...
// ENTITIES THAT EXACT MATCH THE INPUT NODE
    ...
    // ENTITIES THAT EXACT MATCH THE INPUT NODE
    } else if (commands.at(0) =="node_id") {
      std::ostringstream oss;
      oss << g.get_node_id(commands.at(1));
      return_string = oss.str();
    } else if (commands.at(0) =="edge_id") {
      std::ostringstream oss;
      oss << g.get_edge_id(commands.at(1));
      return_string = oss.str();
    } 
...
// NUMBER OF ENTITIES SHARING AT LEAST ONE ONTOLOGY WITH GIVEN ENTITY
      ...
      oss << "\n";
      return_string = oss.str();
    } else if (commands.at(0) == "getpairsbyrel") {
      std::ostringstream oss;
      oss << "\n";
      std::vector<std::tuple<unsigned int, unsigned int, unsigned int, std::string, std::string> > res = g.get_entity_pairs_by_rel(
         commands.at(1), (double) stoi(commands.at(2)));     
      for (auto it = res.begin(); it != res.end(); ++it) {
        oss << std::get<0>(*it) << "\t" << std::get<1>(*it) << "\t" << std::get<2>(*it) << "\t" << std::get<3>(*it) << "\t" << std::get<4>(*it) <<  "\t" << g.get_deg(std::get<0>(*it)) <<  "\t" << g.get_deg(std::get<1>(*it)) << "\n";
      }
      return_string =  oss.str();
      ...
      std::ostringstream oss;
      oss << "\n";
      std::vector<std::pair<unsigned int, std::string> > res = g.get_neighbor_by_rel(commands.at(1),
                                                         commands.at(2), is_true(commands.at(3)));
    ...
    }    else if (commands.at(0) == "neighbor") {
      std::ostringstream oss;
      oss << "\n";
      std::vector<std::tuple<unsigned int, std::string, std::string> > res = g.get_neighbors((unsigned int) stoi(commands.at(1)), (unsigned int) stoi(commands.at(2)), 
                                                         commands.at(3), is_true(commands.at(4)));
    ...
    } else if (commands.at(0) == "getoverlappingpairs") {
      // return_string = std::to_string(g.get_overlapping_pairs(commands.at(1), commands.at(2)));

     std::ostringstream oss;
      oss << "\n";
      std::vector<std::pair<std::string, std::string> > res = g.get_overlapping_pairs(
         commands.at(1), commands.at(2), is_true(commands.at(3)));     
      for (auto it = res.begin(); it != res.end(); ++it) {
        oss << std::get<0>(*it) << "\t" << std::get<1>(*it) << "\n";
      }
      return_string =  oss.str();
