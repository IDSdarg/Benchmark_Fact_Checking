...
class FileUtil extends LazyLogging {
  ...
  def getLineIterator(filename: String): Iterator[String] = Source.fromFile(filename, "UTF-8").getLines
  def getLineIterator(file: File) = Source.fromFile(file, "UTF-8").getLines
  def getLineIterator(stream: InputStream) = Source.fromInputStream(stream, "UTF-8").getLines
