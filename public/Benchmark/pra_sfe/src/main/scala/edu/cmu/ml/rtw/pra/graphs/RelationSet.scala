...
import edu.cmu.ml.rtw.pra.util.Dictionary
import edu.cmu.ml.rtw.pra.util.FileUtil
import edu.cmu.ml.rtw.pra.util.LineFilter
import edu.cmu.ml.rtw.pra.util.IntTriple
import edu.cmu.ml.rtw.pra.util.JsonHelper
import edu.cmu.ml.rtw.pra.util.Pair
...
class RelationSet(baseGraphDir: String, params: JValue, outputter: Outputter, fileUtil: FileUtil = new FileUtil) {
  implicit val formats = DefaultFormats

  // Fields dealing with the relations themselves.

  // The file containing the relation triples.
  val relationFile = s"${baseGraphDir}relation_sets/${(params \ "relation file").extract[String]}"
  ...
  def writeRelationEdgesToGraphFile(
    ...
    var i = 0
    var numEdges = 0
    for (line <- Source.fromFile(relationFile, "ISO-8859-1").getLines) {
