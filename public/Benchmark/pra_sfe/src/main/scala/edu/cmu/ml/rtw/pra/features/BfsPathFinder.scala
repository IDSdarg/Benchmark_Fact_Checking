...
import edu.cmu.ml.rtw.pra.util.Index
...
abstract class BfsPathFinder[T <: Instance](
        ...
        for (relation <- n.edges.keys) {
          ...
          if (inEdges.size + outEdges.size <= maxFanOut) {
            for (i <- 0 until inEdges.size) {
              ...
              if (!shouldSkip(relation, unallowedRelations)) {
  ...
  def shouldSkip(relation: Int, exclude: Set[Int]) = {
    if  (exclude.contains(relation))     
      true
    else
      false
  }
...
class NodePairBfsPathFinder(
  ...
  def getSubgraphForInstance(instance: NodePairInstance) = {
    ...
    // val unallowedEdges = relationMetadata.unallowedPairs
...
class NodePairBfsPathFinder_v2(
  params: JValue,
  relation: String,
  relationMetadata: RelationMetadata,
  outputter: Outputter,
