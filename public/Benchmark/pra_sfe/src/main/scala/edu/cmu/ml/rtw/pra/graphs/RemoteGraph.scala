...
import edu.cmu.ml.rtw.pra.util.SpecFileReader
...
object RunRemoteGraphServer {
  ...
  def main(args: Array[String]) {
    ...
    //   case None => {
    //     (options("graph"), RemoteGraph.DEFAULT_PORT)
    //   }
    // }
    val port = 8080
    val praBase = fileUtil.addDirectorySeparatorIfNecessary(args(0))
    val outputter = new Outputter(JNothing, praBase, "running graph")
    val graph = Graph.create(JNothing, praBase + "/graphs/", outputter, fileUtil).get.asInstanceOf[GraphOnDisk]
