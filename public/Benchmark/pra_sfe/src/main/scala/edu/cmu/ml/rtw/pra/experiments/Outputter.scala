...
class Outputter(params: JValue, praBase: String, methodName: String, fileUtil: FileUtil = new FileUtil) {
  ...
  val baseDir = JsonHelper.extractWithDefault(params, "outdir", praBase)
  ...
  private var name_rel: String = null
  def setRelation(_relation: String, name: String) {
    relation = _relation
    name_rel = name
    // fileUtil.mkdirs(relation + s"/${methodName}")
  }
  ...
  def outputScores[T <: Instance](
    ...
    val filename = relation.replace("scenario", s"${methodName}" + "_score")
  ...
  def outputNodePairScores(
    ...
    writer.write("s" + "\t"+  "o" + "\t" + "p" + "\t" + "true_label" + "\t" + "predict_proba" + "\n")
    scoreStrings.groupBy(_._1).foreach(sourceScores => {
      ...
      for (targetScore <- scores) {
        ...
        if (isPositive) {
          writer.write(source + "\t" + target + "\t" + name_rel + "\t" + "1" + "\t" + score + "\t")
        }
        else {
          writer.write(source + "\t" + target + "\t" + name_rel + "\t" + "-1" + "\t" + score + "\t")
        }
  ...
  def outputWeights(weights: Seq[Double], featureNames: Seq[String]) {
    val filename = relation.replace("scenario", s"${methodName}" + "_weight")
  ...
  def outputPathCounts(pathCounts: Map[PathType, Int]) {
    if (shouldOutputPathCounts) {
      ...
      fileUtil.writeLinesToFile(relation.replace("scenario", s"${methodName}" + "_path_counts"), lines)
    }
  }
  ...
  def outputPathCountMap(
    ...
    if (shouldOutputPathCountMap) {
      val writer = fileUtil.getFileWriter(relation.replace("scenario", s"${methodName}" + "_path_count_map"))
  ...
  def outputPaths(pathTypes: Seq[PathType], graph: Graph) {
    if (shouldOutputPaths) {
      fileUtil.writeLinesToFile(relation.replace("scenario", s"${methodName}" + "_paths"), pathTypes.map(p => getPathType(p, graph)))
    }
  }

  def outputFeatureMatrix(isTraining: Boolean, matrix: FeatureMatrix, featureNames: Seq[String]) {
    if (shouldOutputMatrices) {
      val trainingStr = if (isTraining) "_training_matrix" else "_test_matrix"
      val filename = relation.replace("scenario", s"${methodName}" + trainingStr)
      val writer = fileUtil.getFileWriter(filename)
